...
public class JenkinsMasterImpl extends JenkinsNode implements JenkinsMaster {

	public JenkinsMasterImpl(String hostname) throws IOException {
		...
		for (int i = 0; i < computerJSONArray.length(); i++) {
			...
			if (displayName.startsWith("cloud-")) {
				JenkinsSlave jenkinsSlave = new JenkinsSlaveImpl(
					nodeJSONObject, this);
